...
define(function(require, exports, module) {
    ...
    function _bindEvents() {
        ...
        this._eventInput.on('start', function(event){
            localStorage.setItem("DemoboStorage-scrollviewStart", JSON.stringify(event));
        });
        this._eventInput.on('update', function(event){
            localStorage.setItem("DemoboStorage-scrollviewMove", JSON.stringify(event));
        });
        this._eventInput.on('end', function(event){
            localStorage.setItem("DemoboStorage-scrollviewEnd", JSON.stringify(event));
        });
        this._scroller.on('edgeHit', function(data) {
            localStorage.setItem("DemoboStorage-scrollviewEdgeHit", JSON.stringify(data));
        }.bind(this));

        var self = this;
        window.addEventListener("storage", function(evt) {
            if (evt.key=="DemoboStorage-scrollviewStart") {
                var scrollEvt = JSON.parse(evt.newValue);
                _handleStart.call(self, scrollEvt);
            } else if (evt.key=="DemoboStorage-scrollviewMove") {
                var scrollEvt = JSON.parse(evt.newValue);
                _handleMove.call(self, scrollEvt);
            } else if (evt.key=="DemoboStorage-scrollviewEnd") {
                var scrollEvt = JSON.parse(evt.newValue);
                _handleEnd.call(self, scrollEvt);
            } else if (evt.key=="DemoboStorage-scrollviewEdgeHit") {
